...
class TestEndtoEnd(unittest.TestCase):
    ...
    def test_upd_file(self):
        """The file update test"""
        post_req = requests.post('{}/file'.format(URI),
                                 data=io.BytesIO(b'012345678910'),
                                 params={'filename':'test_upd_file.data'},
                                 stream=True)
        self.assertEqual(post_req.status_code, 201)
        file_id = post_req.json()['file_id']

        req_put = requests.put('{}/file/{}'.format(URI, file_id),
                               params={'filename':'test_upd_file2.data'})
        self.assertEqual(req_put.status_code, 200)

        req_get = requests.get('{}/file'.format(URI))
        self.assertEqual(req_get.status_code, 200)
        self.assertEqual(req_get.json()['content'][0]['filename'], 'test_upd_file2.data')
